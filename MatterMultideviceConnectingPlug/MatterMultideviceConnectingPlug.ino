...
const int TOGGLE_BUTTON_PIN_2 = 3;
const int TOGGLE_BUTTON_PIN_3 = 4;
...
static esp_err_t on_attribute_update(attribute::callback_type_t type,
  ...
  if (type == attribute::PRE_UPDATE && cluster_id == CLUSTER_ID && attribute_id == ATTRIBUTE_ID) {
    ...
    if (endpoint_id == plugin_unit_endpoint_id_1) {
      ...
      digitalWrite(LED_PIN_3, new_state);
    }
  }
  return ESP_OK;
}
...
void setup() {
  ...
  pinMode(TOGGLE_BUTTON_PIN_3, INPUT);
  ...
  attribute_ref_3 =
    attribute::get(cluster::get(endpoint_3, CLUSTER_ID), ATTRIBUTE_ID);
...
void loop() {
  if ((millis() - last_toggle) > DEBOUNCE_DELAY) {
    ...
    if (!digitalRead(TOGGLE_BUTTON_PIN_3)) {
      last_toggle = millis();
      // Read actual on/off value, invert it and set
      esp_matter_attr_val_t onoff_value = get_onoff_attribute_value(attribute_ref_3);
      onoff_value.val.b = !onoff_value.val.b;
      set_onoff_attribute_value(&onoff_value, plugin_unit_endpoint_id_3);
    }    
  }
}